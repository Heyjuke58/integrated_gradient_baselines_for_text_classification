...
def find_next_wrd(wrd_idx, ref_idx, word_features, adj, word_path, strategy="greedy", steps=30):
    if wrd_idx == ref_idx:
    ...
    for j, v in zip(cx.col, cx.data):
        ...
        if strategy == "greedy":
            ...
            monotonic_vec = make_monotonic_vec(
                word_features[ref_idx], word_features[wrd_idx], word_features[j], steps
            )
            ...
            non_mono_count = 10000 - monotonic(
                word_features[ref_idx], word_features[wrd_idx], word_features[j], ret="count"
            )
...
def find_word_path(wrd_idx, ref_idx, word_idx_map, word_features, adj, steps=30, strategy="greedy"):
    ...
    for step in range(steps):
        next_idx = find_next_wrd(last_idx, ref_idx, word_features, adj, word_path, strategy=strategy, steps=steps)
...
def make_monotonic_path(word_path_ids, ref_idx, word_features, steps=30, factor=0):
...
def scale_inputs(
    input_ids, ref_input_ids, device, auxiliary_data, steps=30, factor=0, strategy="greedy"
):
    ...
    for idx in range(len(input_ids)):
        ...
        word_path.append(ref_input_ids[idx])
