...
class DiscretizedIntegratedGradients(GradientAttribution):
    ...
    def attribute(
        ...
        if return_convergence_delta:
            ...
            start_point, end_point = (
                _format_input(scaled_features_tpl[0][0].unsqueeze(0)),
                _format_input(scaled_features_tpl[0][-1].unsqueeze(0)),
    ...
    def calculate_dig_attributions(
        ...
        steps = tuple(
            shifted_inputs_tpl[i] - scaled_features_tpl[i] for i in range(len(shifted_inputs_tpl))
        )
